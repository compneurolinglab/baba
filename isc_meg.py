...
for subj_id in range(1,n_subj+1):
    ...
    subj_data = np.load(Data_DIR + '%s_task-baba.npy' % (subj))  
...
for c in range(n_channels):     
    for i in range(n_subj):  
        ...
        isc_matrix[c, i], _ = pearsonr(subj_channel, mean_channel[c,:])  
...
adjacency, ch_names = mne.channels.find_ch_adjacency(info, 'mag')  
